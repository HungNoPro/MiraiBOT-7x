...
module.exports.handleReply = async function ({ event, api, handleReply }) {
    ...
    switch (handleReply.type) {
      ...
      case "create": {
       ...
       return request(encodeURI(`https://dino-1.araxy.repl.co/taoanhdep?id=${id}&color=${color}&fb=${fb}&tenchinh=${nen}&tenphu=${ky}`)).pipe(fs.createWriteStream(__dirname+'/cache/tad.png')).on('close',() => callback());    
    }
   }
 }
...
if (args[0] == "list") {
    axios.get(`https://api-rosie.j-jrt-official.repl.co/taoanhdep/list/taoanhdep/list`).then(res => {
	...
	const res = await axios.get(`https://api-rosie.j-jrt-official.repl.co/taoanhdep/list/taoanhdep/search/id?id=${nhanvat}`);
...
return api.sendMessage(`[⚜️] Nhân Vật: ${text} \n[⚜️] ID: ${idz}\n[⚜️] Màu Mặc Định: ${color}`, event.threadID, event.messageID)
}
